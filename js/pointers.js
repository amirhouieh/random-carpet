function Pointers( props ){
  ...
  function Pointer( pointerIndex ){
    ...
    this.hasMatch = function(){
      ...
      while( state ){
        var dy = Axis[ 'y' ] ?  Math.floor( Math.abs( this.y - that[ i ].y ) ): true;
        var dx = Axis[ 'x' ] ?  Math.floor( Math.abs( this.x - that[ i ].x ) ): true;

        // var condition =  ( dx && ( dx < neigberBoxSize || dx > 0 ) ) && ( dy && ( dy < neigberBoxSize && dy > 0 ) );
        var condition =  (dx && ( dx < neigberBoxSize && dx > 0 ) ) || ( dy && ( dy < neigberBoxSize && dy > 0 ) );
